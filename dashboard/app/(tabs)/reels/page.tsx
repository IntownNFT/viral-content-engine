...
export default function ReelsPage() {
  ...
  const files = content.reels;

  if (files.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <div className="w-12 h-12 rounded-2xl bg-gray-5/60 flex items-center justify-center mb-4">
          <Film className="h-6 w-6 text-gray-2" />
        </div>
        <p className="text-lg font-medium text-gray-1">No reels generated yet</p>
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h2 className="text-xl font-semibold text-white mt-1">Reel Scripts</h2>
        <p className="text-sm text-gray-1 mt-0.5">{files.length} script{files.length !== 1 ? "s" : ""} generated</p>
      </div>
